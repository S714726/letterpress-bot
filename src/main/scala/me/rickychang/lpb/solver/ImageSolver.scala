...
object ImageSolver {
  ...
  def findWords(imageFilePath: String, playedWords: Set[String], maxWords: Int): List[String] = {
    val boardImage = ImageIO.read(new File(imageFilePath))
    val imageParser = new MultiDeviceParser(tileParser)
    val wordsToPlay = boardSolver.findMoves(imageParser.parseGameBoard(boardImage), playedWords, maxWords).map {
